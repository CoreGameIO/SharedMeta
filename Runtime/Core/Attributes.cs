...
namespace SharedMeta.Core
{
    ...
    /// <summary>
    /// How much of a result value the generated desync diagnostics print.
    /// </summary>
    public enum DesyncValueDetail
    {
        /// <summary>
        /// Dump every public member, recursing into nested types and listing collection
        /// elements (bounded — 3 levels deep, 8 elements per collection).
        /// </summary>
        Full = 0,

        /// <summary>
        /// Dump scalar members only. Nested types collapse to <c>&lt;TypeName&gt;</c> and
        /// collections to an element count. Keeps desync log lines short on states whose
        /// result DTOs carry large graphs.
        /// </summary>
        Short = 1,
    }

    /// <summary>
    /// Project-level knobs for generated diagnostics.
    /// <para>Example:</para>
    /// <code>
    /// [assembly: SharedMeta.Core.SharedMetaDiagnosticsOptions(DesyncValues = SharedMeta.Core.DesyncValueDetail.Short)]
    /// </code>
    /// </summary>
    [AttributeUsage(AttributeTargets.Assembly)]
    public sealed class SharedMetaDiagnosticsOptionsAttribute : Attribute
    {
        /// <summary>
        /// Detail level of result values in desync log lines and
        /// <see cref="SharedMeta.Core.Diagnostics.DesyncException"/> messages.
        /// Default <see cref="DesyncValueDetail.Full"/>.
        /// <para>
        /// The generator emits a per-type formatter for every method result type, because
        /// the default <c>object.ToString()</c> prints only a type name — the two sides of a
        /// desync then look identical in the log while their bytes differ. Types that
        /// override <c>ToString()</c> (records included) keep their own rendering.
        /// </para>
        /// </summary>
        public DesyncValueDetail DesyncValues { get; set; } = DesyncValueDetail.Full;
    }

    /// <summary>
    /// Restricts a method to callers holding at least one of the named permissions. Permissions
    /// are entitlements of the account — <c>Cheat</c>, <c>Admin</c>, <c>Support</c> — resolved
    /// server-side per player and never writable from shared game logic.
    /// </summary>
    /// <remarks>
    /// On an interface the requirement applies to every method it declares; a method's own
    /// attribute replaces the interface's rather than adding to it, so a single method can be
    /// opened up or tightened without splitting the service.
    /// <para>
    /// Deliberately NOT the tool for in-game roles (clan leader, officer, party host). Those are
    /// properties of a (player, entity) pair, they live in the entity's own state, and they change
    /// as a result of game logic — so they belong in a check against that state, which both sides
    /// evaluate identically and replay reproduces. A permission is an account-level fact with a
    /// different source of truth, a different lifetime, and a write path players must never reach.
    /// </para>
    /// <code>
    /// [RequirePermission("Cheat")]
    /// void GrantGold(int amount);
    ///
    /// [RequirePermission("Admin", "Moderator")]   // any one of them suffices
    /// void MuteMember(string playerId, int minutes);
    /// </code>
    /// </remarks>
    [AttributeUsage(AttributeTargets.Method | AttributeTargets.Interface)]
    public sealed class RequirePermissionAttribute : Attribute
    {
        /// <summary>Permissions accepted for this method. Holding any one of them admits the call.</summary>
    ...
    /// <summary>
    /// The permissions this game recognizes. Optional: without it any string is accepted, and a
    /// typo silently becomes a permission nobody holds — a method no caller can ever reach.
    /// Declared, it turns that typo into a build error naming the method.
    /// <para>Example:</para>
    /// <code>
    /// [assembly: SharedMeta.Core.DeclaredPermissions("Cheat", "Admin", "Support")]
    /// </code>
    /// </summary>
    /// <remarks>
    /// Order is irrelevant. The server assigns each permission a bit by sorted position over the
    /// union of every name it knows, so adding or rearranging declarations never moves an existing
    /// permission's bit — and that bit is server-internal, computed from the server's own build and
    /// never shipped for a client to index into.
    /// </remarks>
    [AttributeUsage(AttributeTargets.Assembly, AllowMultiple = true)]
    public sealed class DeclaredPermissionsAttribute : Attribute
    {
        /// <summary>Permission names declared by this attribute instance.</summary>
        public string[] Permissions { get; }

        public DeclaredPermissionsAttribute(params string[] permissions)
        {
            Permissions = permissions ?? Array.Empty<string>();
        }
    }
