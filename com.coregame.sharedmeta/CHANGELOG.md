...
- **Always-`Async` naming** across every generated API surface — client `ApiClient`, cross-entity `EntityCaller` interface, sibling/recorder/replayer/LocalEntityCaller impls. Replaces the prior split rule that preserved Task-source names on EntityCaller (so server-side code wrote `await GetIClanService(id).AddPower(delta);`) but always suffixed on the client (`await api.AddPowerAsync(delta);`). Now both sides emit `XAsync` regardless of source `void` / `Task` / `Task<T>`, with dedup so a source already named `XAsync` doesn't double-suffix. Switching a method between `void` ↔ `Task` ↔ `Mode = Notification` ↔ `Mode = Server` no longer renames the generated method. **Migration (source-breaking):** existing server-side cross-entity calls that used the raw source name need `Async`: `GetIClanService(id).AddPower(...)` → `AddPowerAsync(...)`. Client `ApiClient` usage is unchanged (it already always had `Async`).
- Cross-entity caller for `Mode = ExecutionMode.Notification` methods now returns `Task` (was `void`). Switching a method between `Server` and `Notification` no longer touches the existing `await GetIClanService(id).AddPowerAsync(delta);` call sites — the same async shape works under both modes. The returned Task awaits the Orleans `[OneWay]` **send-flush** (dispatch to the target silo's task pool) — not the receiver's processing — so callers can sequence subsequent local work after the dispatch without waiting for a reply. All five EntityCaller surfaces (interface, sibling, server-side recorder, client replayer, CrossOptimistic local) emit consistent `Task XAsync`; the recorder chain-returns the `CallEntityOneWay` Task instead of swallowing it. **Migration:** existing Notification call sites that ignored the void return now produce `CS4014` — add `await` or `_ =`.
- Cross-entity handler triad on `ServerMetaContext` / `MetaProviderBase` switched from inline `Func<string, ushort, ReadOnlyMemory<byte>, long, …>` to three named delegates in `SharedMeta.Server`: `EntityCallHandler`, `EntityCallOneWayHandler`, `EntityStateHandler`. One declaration replaces the same long signature repeated across three files; parameter names propagate through IntelliSense; signature drifts caught in one place. Fixes a latent type mismatch where `MetaProviderBase.EntityCallOneWayHandler` was still `Action<>` while `ServerMetaContext` had already moved to `Func<…, Task>` (would have surfaced as a compile error the moment the assignment was exercised).
- `EntityGrain` cross-entity wiring extracted from four inline lambdas in `OnActivateAsync` to instance methods `HandleCrossEntityCallAsync`, `HandleCrossEntityOneWayAsync`, `LoadCrossEntityStateAsync`, `ForcePersistStateAsync`. No allocation of four `<>c__DisplayClass…` closures per activation; named frames in stack traces; `await`/`_ = ` discard fixed on the OneWay path so the Orleans-dispatch Task chains through to the caller.
