...
**`MetaRef<T>` (0.40.0+) — the handle to store; never cache an ApiClient.** `DisconnectAsync` and the supersede session restart (`ClearAllConnections`) dispose every ApiClient under the connection and drop it; nothing re-subscribes automatically, so a field or DI singleton holding the instance points at a disposed object. `MetaRef<T>` holds the key `(TApiClient, entityId)` and re-resolves per access — `Current` / `TryGet(out api)` are the alloc-free `TryGetService` path, `GetAsync()` is a `ValueTask<T>` that completes synchronously once subscribed. Build via `client.PlayerRef<TApi>()` (follows `PlayerId` across relogin) or `client.Ref<TApi>(entityId)`. Generated per-assembly `MetaRefs` container: one property per service — `MetaRef<T>` for UserOwned, `MetaRefFactory<T>` for the rest (`.For(entityId)` when the id arrives; allocates a handle per call, so hold it). **State on a loop:** `GetState<T>` throws when not subscribed — use `client.TryGetState<T>(entityId, out var s)` / `resolver.TryGetState` in anything polling per tick or per frame (an ECS barrier, a UI binding). A handle carries the client, not the state: `MetaRef<TApi>` does not know the entity's `TState`, so state access goes through the client. Host creates the client inside an async connect / replaces it on re-bootstrap → `new MetaRefs(() => svc.Client)`; the container registers before the client exists, handles report a miss until it does and follow the current one after. DI, preferred: `refs.Accept(IMetaRefVisitor)` — visits every service with its API client type as a real generic argument (`Visit<TApi>(MetaRef<TApi>)` / `Visit<TApi>(MetaRefFactory<TApi>)`), so a container binder is two methods with no `Type` and no `object`. Fallback for containers that only resolve lazily by type: `refs.TryGet(Type, out object)` (declines unknown/null instead of throwing) and `refs.All`. Handles are pre-built and type-indexed, so no `MakeGenericType` — AOT-safe. Composition root names no service, and adding one needs no registration change. `client.ConnectionInvalidated += (entityId, stateType) => …` fires after a connection is dropped and its clients disposed (not on `Dispose()`); that's where re-subscribe belongs.

**Stale-client failures are loud (0.40.0+).** Every mutating call on an API client whose entity was disconnected throws `ObjectDisposedException` (service + method + entity) — `Server`/`ServerPatch`/`ServerReplace`/`Optimistic`/`CrossOptimistic`/`Signal`. The guard is at method entry, not at the send: Optimistic mutates local state first, so a later check would leave an undeliverable mutation applied; it refuses with local state untouched. Still permitted: `LocalQuery` reads and properties (`HasError`, `ClearError`, `MutationCount`) — teardown/recovery code needs them, so a read-only stale client still returns data frozen at the disconnect with no error. Separately, a send that fails for other reasons (transport dropped) is logged via `OptimisticSend.ReportIfFailed` — Optimistic has already returned, so logging is the only channel. Root cause fixed alongside: `DispatcherNetworkAdapter` never implemented `IDisposable` while `EntityConnection.Dispose` gates on `Network is IDisposable`, so adapter cleanup had never run — leaking a dispatcher broadcast subscription + `OnDisconnected` handler per disconnected entity.
...
**Reconnect auth recovery (0.37.2+):** auth rejection at `SessionConnect` is now structural — the SignalR hub returns `SessionConnectResponse { Success = false, FailureReason = AuthenticationRequired }` instead of throwing `HubException("Authentication is required")` (breaking for anyone catching the exception by message); HTTP polling keeps 401 and ships the same body; since 0.37.3 all three polling clients (`HttpPollingConnection`, `BestHttpPollingConnection`, `UnityHttpConnection`) parse a 401 on `/session-connect` instead of throwing, and synthesize the same answer via `SessionConnectResponse.AuthenticationRequired(...)` when the body is empty (an ASP.NET `.RequireAuthorization()` middleware 401 carries none). Every other status still throws. `ClientDispatcher.ReconnectAsync` handles it: re-acquire via `AuthRecoveryHandler` (set by `MetaClient` from `AccessTokenSource` / `OnConnectAuthFailedAsync`), re-dial the transport — mandatory, SignalR resolves its token during the handshake, not per invocation — then retry the handshake once. Before 0.37.2 the 0.30.1 recovery was wired only to `MetaClient.ConnectAsync`, and the reconnect path (driven by a transport event, bypassing `MetaClient`) dead-ended at `ConnectionStatus.Failed` — the classic trigger being an app resumed from background with an expired token. Requires a provider-based connection (`tokens.GetTokenAsync`); a fixed token string is re-sent verbatim forever. `IdentityUnknown` is deliberately **not** retried (a new token means a different PlayerId, which a live session can't adopt) → `Failed`, game restarts its login flow. `AuthFailureHeuristic.LooksLikeAuthFailure` (`SharedMeta.Core.Auth`) is the message-based fallback for pre-0.37.2 servers and client-side 401 throws.
