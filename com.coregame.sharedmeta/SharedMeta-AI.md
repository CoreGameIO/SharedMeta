...
> **0.27.0+ / 0.27.1**: `services.ConfigureConfigs(o => { o.UseDefaultInstances("0.1.0"); o.Strategy = ConfigSeedStrategy.LoadIfNew; });` wires the whole publish-side: per-`[MetaConfig]` `BroadcastingConfigProvider<T>` (auto-emitted by generator), two-phase `IConfigBootstrapper` (`GetVersionAsync` → strategy gate → `GetBytesAsync`), audit through `IConfigMetadataGrain`, observer fan-out warm-up. Built-ins: `DefaultInstanceConfigBootstrapper` (in-memory `Activator.CreateInstance` + `IMetaSerializer`, works in read-only Docker) and `DirectoryConfigBootstrapper` (read-only `{root}/{Type.Name}/{M.m.p}.bin` scan). Project pre-bootstrap work (dev YAML compile, etc.) registers as its own `IHostedService` before `ConfigureConfigs` — `OnBeforeSeed` and `UseLoader` were removed in 0.27.1 (use the interface directly or a project HostedService). Admin tools call `IConfigAdminGrain` directly from a connected Orleans client — operations: `ListConfigsAsync` / `GetConfigAsync` / `DownloadAsync` / `UploadAsync` / `UnpublishAsync` + client-version control (`Get` / `Set{Current,Min,Max}ClientVersionAsync`). `ClientVersionOptions` (`appsettings.json "ClientVersion"` section: `Current`/`Min`/`Max`/`Server`) seeds `ICurrentClientVersionGrain` + `MetaTransportOptions`; the auto-registered `DefaultClientVersionService` (`IConfigVersionResolver` + `IHostedService`) tracks runtime grain overrides via 30-s poll for cross-silo propagation.
