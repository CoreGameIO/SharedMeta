...
- **Breaking:** framework subscriber interfaces are gone as a mechanism. `ILobbySubscriber` is now an ordinary contract carrying `[MetaServiceContract]`: inherit it on the `[MetaService]` interface and mark each implementing method `[MetaMethod(Mode = ExecutionMode.Server, GenerateClientApi = false)]` on the impl class. Its methods get a real game method id, a dispatcher case and client-side replay like any other meta method. Migration: drop `SubscriberInterfaces = new[] { typeof(ILobbySubscriber) }` from `[MetaService]`, add `, ILobbySubscriber` to the interface's base list, add `[MetaMethod]` to the three impl methods.
- `[MetaServiceContract]` — marks an interface a meta service can inherit so server code can address that service by contract. The generator emits an awaitable mirror `I{Contract}ServerApi` **into the contract's own assembly**, which is what lets code that cannot reference the game make a typed call:
  ```csharp
  public LobbyGrain(ILogger<LobbyGrain> logger, ILobbySubscriberServerApi? players = null) { ... }
...
- `{Service}ServerApi` — generated typed API for invoking a service's meta methods from server code that isn't itself inside a meta call (admin tooling, framework grains, background jobs): `grainFactory.GetServerApi<IProfileService>(entityId).AddResourcesAsync(...)`. Routes through the server-internal cross-entity entry, so replay recording, broadcast to subscribers, persistence and sequence advance all happen; works on a cold entity with no subscriber. Emitted into shared projects behind `SHAREDMETA_SERVER` and into server projects from referenced assemblies.
- `IMetaServerApiFactory` — DI handle for the server API, registered automatically. Inject it into an admin grain or endpoint and call `factory.GetServerApi<IMyService>(entityId)`: the call shape is identical whichever serializer the service's assembly declared, instead of demanding an `IMetaSerializer` argument under `[assembly: MetaSerializer(SerializerType.Generic)]`. The `IGrainFactory` extensions remain for callers holding no service provider.
- `grainFactory.GetServerApi<IService>(entityId, callerClientVersion)` — optional app version to run the call under, deciding the config branch the body executes against and how far state migration may advance. Omit it to let the entity resolve its own; pass one to deliberately act as a specific client build (e.g. driving a staged migration ahead of the owning player's version).
- `[MetaMethod]` on a `[MetaServiceImpl]` method now adds it to the service's method surface, so a service can inherit its contract from a base interface it doesn't own (id assignment, dispatcher, client broadcast replay all follow). Signature drift stays a compile error because the class must implement the inherited member.

### Fixed

- **Security:** methods with `Mode = ExecutionMode.Notification` were not gated server-side. The implicit "clients never originate notifications" rule lived only in the client API generator, so no client method was generated but the dispatcher emitted no `IsClientCall` guard and the server signature advertised the method as client-callable — a forged packet carrying its id was dispatched. The rule now has one implementation (`MetaMethodFacts`) shared by the dispatcher, the client generator and discovery.
- Server-originated calls (cross-entity, `{Service}ServerApi`, lobby notifications) now honour force-patch fan-out. Previously only client RPCs computed `requirePatchForFanOut`, so a subscriber needing the ServerPatch fallback received a replay-only broadcast and replayed a method body it was known to disagree with.
- `{Service}ServerApi` picks its serializer from the assembly that **declares** the service, not from the project being compiled. A server project referencing MemoryPack while its shared assembly does not produced raw MemoryPack argument bytes against a dispatcher reading the length-prefixed `IPayloadWriter` format — the reader consumed a length prefix out of member data and handed the method a plausible-looking object with empty collections. Silent, and invisible to client calls, since these are exactly the methods clients cannot reach.
- `[ServiceConfig]` accessors resolve their config by type instead of by a service-local index. `Context.Configs` holds the union of every service sharing a state, so on a multi-service state the index addressed a sibling's config — an `InvalidCastException`, or silently the wrong config where the types matched. Applies to both the server-side accessor and the client `ApiClient` one.
- Config version resolution substitutes `IConfigVersionResolver.CurrentClientVersion` on every path when the caller has no client version, not just in the cached-config getters. `ResolveClientConfigVersion`, `EstablishConfigPinsFromClientVersion` and the per-`[ServiceConfig]` version resolver each threw "clientAppVersion is required" for server-originated calls into a config-bound service.
- A server-originated call now runs under **the entity owner's app version** — taken from the persisted subscribers, so it still answers for an offline player — instead of the server's current client version. Acting on a player's entity under the server's latest branch would execute logic that player's build may not have, and could drive a state migration their client cannot read. Every config the call resolves comes from that one version, which is what keeps a multi-config entity internally consistent. Several subscribers on a shared entity resolve to the lowest version; entities that never had one fall back to the server default.
- Configs no longer have to be pre-cached to be usable: a server-originated call materializes the config for its effective version (`InitializeConfigAsync` / `InitializeConfigsAsync`) before the synchronous getters run. Async-backed providers previously threw "no cached config for X" for any version nobody had subscribed under.
- A direct RPC to an entity that a `CrossOptimistic` call had targeted no longer hangs. The session's sequence bookkeeping for a cross-call target was keyed under an empty state type since 0.33.0, so the target's grain sequence ran away from the tracked value one step per cross-call; the next direct call to it read as a sequence gap, deferred its response forever, and the client's request never resolved (then re-sent on timeout and came back as "processed by a previous server instance"). `ServerMethodEntry.StateTypeName` gives the server the methodId→state-type mapping the fix needs.
- A host that registers no `IConfigVersionResolver` no longer logs an error per entity activation — its absence is a normal configuration, now logged at Debug.
