...
namespace SharedMeta.Generator.Generators
{
    public static class ContextInjectionGenerator
    {
        public static string? Generate(ClassDeclarationSyntax node, INamedTypeSymbol symbol, Compilation compilation)
        {
            ...
            // 0.33.0+ [ServiceConfig] accessors — one named property per declared entry,
            // positional (declaration order = Context.Configs index). Symmetric/independent —
            // never merged with the legacy Config or with each other's branch.
            if (serviceInterfaceSymbol != null)
            {
                var serviceConfigs = serviceInterfaceSymbol.GetAttributes()
                    ...
                    .ToList();
                for (int i = 0; i < serviceConfigs.Count; i++)
                {
                    var (scType, scName) = serviceConfigs[i];
                    var scTypeName = scType!.ToDisplayString();
                    sb.AppendLine();
                    // Settable backing field mirrors the legacy Config property's shape —
                    // needed so Get{Iface}SiblingAsync() (multi-config sibling resolution) can
                    // pin THIS instance's value without touching the shared Context, exactly
                    // like it already does for the legacy primary Config.
                    sb.AppendLine($"        private {scTypeName}? _serviceConfig_{scName};");
        ...
        internal static void GenerateSiblingAsyncGetter(
            ...
            List<(string TypeName, string Name)>? depServiceConfigs = null)
        {
            depServiceConfigs ??= new List<(string TypeName, string Name)>();
            var interfaceName = depSymbol.Name;
            var interfaceFqn = depSymbol.ToDisplayString();
            var depNamespace = depSymbol.ContainingNamespace.ToDisplayString();
            var baseName = interfaceName;
            ...
            sb.AppendLine($"#if SHAREDMETA_SERVER");
            bool hasAnyConfig = depConfigTypeName != null || depServiceConfigs.Count > 0;
            string asyncModifierServer = hasAnyConfig ? "async " : "";
            string returnExprServer = hasAnyConfig ? "return sibling;" : "return System.Threading.Tasks.Task.FromResult(sibling);";
            ...
            // 0.33.0+ [ServiceConfig] entries — each resolved independently through its own
            // provider and pinned to THIS sibling instance's settable backing field (see the
            // per-entry property emitted in ContextInjectionGenerator's accessor block), same
            // "don't touch the shared Context" contract the legacy Config branch above uses.
            foreach (var (scTypeName, scName) in depServiceConfigs)
            {
                sb.AppendLine($"            {{");
                sb.AppendLine($"                var _scProvider = Context.ResolveService<SharedMeta.Server.Core.IMetaConfigProvider<{scTypeName}>>();");
                sb.AppendLine($"                var _scPolicyResolver = SharedMeta.Core.MetaConfigVersionResolver.ForType(typeof({scTypeName}));");
                sb.AppendLine($"                var _scResolvedVersion = _scProvider.ResolveForClient(Context.CallerClientVersion, _scPolicyResolver);");
                sb.AppendLine($"                var _scTypedConfig = await _scProvider.GetConfigAsync(_scResolvedVersion);");
                sb.AppendLine($"                if (sibling is {implFqn} __siblingImpl_{scName}) __siblingImpl_{scName}.{scName} = _scTypedConfig;");
                sb.AppendLine($"            }}");
            }
        ...
        /// <summary>
        /// 0.33.0+ Read the dep service's <c>[ServiceConfig(typeof(TConfig), "Name")]</c> entries,
        /// declaration order — the symmetric counterpart to <see cref="ReadDepConfigType"/>.
        /// Empty when the dep declares none.
        /// </summary>
        internal static List<(string TypeName, string Name)> ReadDepServiceConfigTypes(INamedTypeSymbol depInterface)
        {
            return depInterface.GetAttributes()
                .Where(a => a.AttributeClass?.ToDisplayString() == "SharedMeta.Core.ServiceConfigAttribute")
                .Select(a => (
                    Type: a.ConstructorArguments.Length > 0 ? a.ConstructorArguments[0].Value as INamedTypeSymbol : null,
                    Name: a.ConstructorArguments.Length > 1 ? a.ConstructorArguments[1].Value as string : null))
                .Where(e => e.Type != null && !string.IsNullOrEmpty(e.Name))
                .Select(e => (e.Type!.ToDisplayString(), e.Name!))
                .ToList();
        }
