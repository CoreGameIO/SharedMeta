...
namespace SharedMeta.Generator.Generators
{
    ...
    public static class ServiceRegistrationGenerator
    {
        public static string? Generate(InterfaceDeclarationSyntax node, INamedTypeSymbol symbol, Compilation? compilation = null)
        {
            ...
            // Method parameter and return types are written into the replay dispatcher exactly as
            // the interface spelled them, so this file needs whatever the interface had in scope.
            // The ApiClient and the server dispatcher render the same signatures and carry the same
            // two usings; leaving them out here made a `List<T>` parameter break only this file.
