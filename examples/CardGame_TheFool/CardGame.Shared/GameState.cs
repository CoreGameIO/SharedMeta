...
namespace CardGame.Shared
{
    ...
    {
        [MemoryPackOrder(0)] public Card AttackCard { get; set; } = null!;
        [MemoryPackOrder(1)] public Card? DefenseCard { get; set; }
    }

    [MemoryPackable]
    public partial class GameState : ISharedState
    {
        [MemoryPackOrder(0)] public List<Card> Deck { get; set; } = new();
        [MemoryPackOrder(1)] public List<List<Card>> PlayerHands { get; set; } = new();
        [MemoryPackOrder(2)] public List<Player> Players { get; set; } = new();
        [MemoryPackOrder(3)] public Card? TrumpCard { get; set; }
        [MemoryPackOrder(4)] public Suit TrumpSuit { get; set; }
        [MemoryPackOrder(5)] public bool GameStarted { get; set; }
        [MemoryPackOrder(6)] public int? CurrentAttackerId { get; set; }
        [MemoryPackOrder(7)] public int? CurrentDefenderId { get; set; }

        /// <summary>Current game phase.</summary>
        [MemoryPackOrder(8)] public GamePhase Phase { get; set; } = GamePhase.NotStarted;

        /// <summary>Cards currently on the table (attack/defense pairs).</summary>
        [MemoryPackOrder(9)] public List<TablePair> Table { get; set; } = new();

        /// <summary>Player IDs who exited the game (hand empty when deck was empty).</summary>
        [MemoryPackOrder(10)] public List<int> Winners { get; set; } = new();

        /// <summary>Max cards per hand (refill target).</summary>
        [MemoryPackOrder(11)] public int MaxHandSize { get; set; } = 6;

        /// <summary>
        /// Maps client connection ID to player ID.
        /// Used to identify which player is making a request.
        /// </summary>
        [MemoryPackOrder(12)] public Dictionary<string, int> ClientToPlayer { get; set; } = new();

        /// <summary>
        /// Maps player ID to profile entity ID.
        /// Used to notify profiles about game results.
        /// </summary>
        [MemoryPackOrder(13)] public Dictionary<int, string> PlayerToProfileEntity { get; set; } = new();

        /// <summary>
        /// Game entity ID (for result notifications).
        /// </summary>
        [MemoryPackOrder(14)] public string GameEntityId { get; set; } = "";

        /// <summary>
        /// Player ID of the loser (the fool). Null if game not over or draw.
        /// </summary>
        [MemoryPackOrder(15)] public int? LoserId { get; set; }
    }
