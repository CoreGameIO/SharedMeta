...
**`IMetaSerializer.Pack<T>(T)` returns `ReadOnlyMemory<byte>`.** Stock codec wraps a fresh `byte[]`; the per-grain cached `IPayloadWriter` (§4.6) writes into a pool-rented buffer and returns a slice valid until the next `Reset`. Callers that need ownership do `.ToArray()` at the boundary (persistence, sites that outlive the current `Handle*Async` frame).
...
### 4.6 Hot-Path Allocation Strategy (0.23.0+, simplified in 0.24.0)

Server-side RPC dispatch has two structural sources of per-call allocation: outgoing wire payloads (broadcast variants × N receivers) and intermediate serializations (dispatcher result, patch tree, state pack, triggers). Both are addressed without changing the user-visible call shape.

**Wire-DTO shape: `ReadOnlyMemory<byte>` (0.23.0 wire-breaking).** Every hot-path DTO byte field (`MetaOperation.{Payload,ResultBytes,ReplayPayload,PatchBytes,StateBytes}`, `EntityBroadcast.OpBytes`, `EntityCallResult.OpBytes`, `SessionOp.OpBytes`, `CrossEntityCallReturn.ResultBytes`, …) carries `ReadOnlyMemory<byte>` instead of `byte[]?` / `ImmutableArray<byte>`. ROM rides the wire without an extra copy at every Orleans hop; `Orleans.Concurrency.Immutable<T>` is applied at grain method boundaries that own the buffer (skip-copy on the in-silo hop).

**Cached `IPayloadWriter` per grain.** `ServerMetaContext` holds two reusable writers — one for the replay payload (the `MetaOperation` written during `Handle*Async`) and one for cross-entity args — and resets each between operations:

```csharp
public void BeginOperation()  => _writer.Reset();
public ReadOnlyMemory<byte> EndOperation() => _writer.Complete();
public IPayloadWriter AcquireWriter() { _crossEntityArgsWriter.Reset(); return _crossEntityArgsWriter; }
```

`IPayloadWriter.Complete()` returns `ReadOnlyMemory<byte>` over an `ArrayPool<byte>`-rented buffer; ROM is valid until the next `Reset` (pooled mode, used in-grain) or `Dispose` (transient mode, used by ad-hoc test code). `IMetaSerializer.Unpack<T>(ReadOnlySpan<byte>)` is the sync codec entry — saves one Span-accessor hop vs the ROM overload, ref-struct-clean.
...
**`SessionManagerGrain` per-tick allocs.** `QueuedBroadcast`, `DeferredResponse`, `EntitySubscriptionInfo`, `StashedRpcCall` are structs (stored in `List<T>` / `Dictionary<TKey,TStruct>`, no per-add heap object). `_pendingPackets` is an expression-bodied alias to `_persistentState.State.PendingPackets!` — eliminates the parallel-list synchronization problem. `CleanupPendingPacketsBySequence` is a prefix-scan + single `RemoveRange(0, k)` (O(N−K) one-shift, replaces O(N·K) per-element `RemoveAt`). `BuildPrecedingOperations` writes directly into the destination `List<SessionOp>` (no intermediate list). `DrainHeldBroadcasts` walks the `SortedDictionary` via its struct enumerator instead of `.First()` (avoids the `IEnumerator<>` box).

**What this replaces.** 0.23.1 introduced `PooledPayloadRegistry` — a silo-scoped ref-counted byte-buffer pool with a 5-bit silo / 27-bit slot encoding (`PooledPayload.Ref`), a cluster-singleton coordinator grain handing out unique `SiloId`s on bootstrap, and ownership tokens threaded through `HandleCallResult`. It shipped opt-in (`PooledPayloadOptions.UsePoolPath` default false, `SHAREDMETA_ENABLE_POOL` env var) and was never engaged in measured stress runs — the cached-writer pattern above already amortises the `byte[]` allocation across calls without lifetime discipline at the consumer level. 0.24.0 removed the registry, the coordinator grain, the startup task, and the ownership tokens (~1500 LOC across 9 files). The ROM-shape wire DTOs survived; that was the load-bearing piece.
...
**Cross-build translation.** `MetaServerSignature` and `MetaClientSignature` both carry `GlobalIndex` per method entry. During `SessionConnect` phase-2 (`RegisterClientSignature`), `MetaConnectionHandler` builds two maps: `clientToServer[clientId] = serverId` (rejected methods get `ushort.MaxValue` sentinel) and `serverToClient[serverId] = clientId` (shipped to the client as `ClientSignatureAnnotated.ServerToClient[]` since 0.24.0; was `ClientCapabilities.ServerToClientMethodIds` in 0.22.x/0.23.x). RPC dispatch translates on receive; broadcasts translate on send.
...
|---|---|
| 0.24.0 | **Signature handshake redesign** — `ClientCapabilities` / `MethodSignatures` removed; server ships `ClientSignatureAnnotated` (verdict + id-translation array, O(1) gate, < 100 B steady-state connect). Client-side `IServerAnnotationCache` (in-memory + Unity PlayerPrefs) keyed by `clientHash`, invalidated when `serverHash` shifts. **Client-owned subscriptions** — server no longer persists subscription list; client claims its set on Resume, entity grain verdicts `Continued` / `Refreshed` / `Failed` per claim (`EntityGrain.ReclaimSubscriptionAsync`). `Reclaim` truth source is `EntitySequenceNumber`; missing `Subscribers` entry is repaired in place. **Session recovery** — explicit `SessionConnectMode.{Resume,StartNew}`, `IMetaSessionRecoveryHandler` game-level callback on `SessionUnknown`, unified `TriggerRecovery` (transport-reconnect + server-pushed). Persisted RPC-ordering baseline (`LastDispatchedRequestId` + client-reported `LastCompletedRequestId`) eliminates infinite retry loops after silo restart. **Allocation rework** — `PooledPayloadRegistry` (0.23.1 opt-in, never engaged) removed (~1500 LOC); replaced by cached `IPayloadWriter` per grain over `ArrayPool<byte>` scratch (`IServerRecordContext.AcquireWriter()`); `Immutable<ROM>` kept only at Orleans grain boundaries; `SessionManagerGrain` hot-path wrappers flipped class → struct; `_pendingPackets` aliased to persisted state. Client `_lastKnownEntitySeq` advances on every `CrossEntityOperations[i]`, not just the outer entity. **Wire-breaking**: `SessionConnectRequest`/`SessionConnectResponse` field additions + removal of `ResubscribedEntities` and `MethodSignatures`/`SignatureMismatches` |
...
- **Serialization:** `IMetaSerializer` interface — new serializers plug in without touching transport or game logic. The `IPayloadWriter` lifecycle (Pack into a pool-rented buffer, return ROM, `Reset` between uses) keeps the decorator boundary cheap (§4.6)
